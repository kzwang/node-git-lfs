    ...
    return {
        ...
        actions: {
            upload: {
                ...
                expires_at: _getJWTExpireTime(),
                header: {
                    "Authorization": "JWT " + generateJWTToken('upload', user, repo, oid)
                }
            },
            verify: {
                ...
                expires_at: _getJWTExpireTime(),
                header: {
                    "Authorization": "JWT " + generateJWTToken('verify', user, repo)
                }
            }
        }
    }
    ...
    if (exist) {
        result.actions = {
            download: {
                ...
                expires_at: _getJWTExpireTime(),
                header: {
                    "Authorization": "JWT " + generateJWTToken('download', user, repo, oid)
                }
            }
        }
    } else {
 ...
 */
var generateJWTToken = function(action, user, repo, oid) {
    var signObject = {
        user: user,
        repo: repo,
        action: action
...
var _getJWTExpireTime = function() {
