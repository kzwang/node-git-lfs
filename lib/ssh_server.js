...
hostKeys = config.has('ssh.key.passphrase') ? [{ key: fs.readFileSync(config.get('ssh.key.private')), passphrase: config.get('ssh.key.passphrase') }] : [fs.readFileSync(config.get('ssh.key.private'))];
        ...
        client.on('session', function(accept, reject) {
            ...
            session.once('exec', function(accept, reject, info) {
                ...
                var res = {
                    ...
                    href: PORT ? `${BASE_URL}:${PORT}/${commands[1]}` : `${BASE_URL}/${commands[1]}`
